...
import torch
from app.src.agglo_clustering import AggloClustering

from app.src.sentence_embedder import SentenceEmbedder
from app.src.token_embedder import TokenEmbedder
...
def apply_seems(example_name, n_clusters=None):
    ...
    summaries = pd.read_csv(summaries_path).sort_values(
        by="pyramid", ascending=False).reset_index(drop=True)
    ...
    for index, row in summaries.iterrows():
        ...
        cluster_affinity_matrix = utils.cos_sim(inference_embedder.embeddings.cpu(
